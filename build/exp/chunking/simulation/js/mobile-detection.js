/**
 * Mobile Detection and Orientation Alert for Chunking Experiment
...
(function () {
  ...
  function debugLog(message, data = null) {
    if (CONFIG.enableDebugMode) {
      console.log(`[Chunking-MobileDetection] ${message}`, data || "");
    }
  }
  ...
  function hasUserDismissedAlert() {
    ...
    if (sessionStorage) {
      return (
        sessionStorage.getItem("chunking-mobile-alert-dismissed") === "true"
      );
    }
  ...
  function markAlertAsDismissed() {
    ...
    if (sessionStorage) {
      sessionStorage.setItem("chunking-mobile-alert-dismissed", "true");
    }
  ...
  function resetDismissalState() {
    ...
    if (sessionStorage) {
      sessionStorage.removeItem("chunking-mobile-alert-dismissed");
    }
  ...
  function createAlertElement() {
    ...
    overlay.id = "chunking-mobile-orientation-alert";
    ...
    // Add CSS animations
    if (!document.getElementById("chunking-mobile-alert-styles")) {
      const style = document.createElement("style");
      style.id = "chunking-mobile-alert-styles";
      style.textContent = `
                ...
                .chunk-animation {
                    animation: chunkFlow 2s ease-in-out infinite;
                }
    ...
    heading.textContent = "Phrase Chunking Works Best in Landscape";
    ...
    expTitle.textContent = "Text Chunking Experiment";
    ...
    description.innerHTML = `
            This experiment involves <strong>phrase pattern recognition</strong> and chunk boundary identification. For optimal experience with regex patterns and text analysis:
        `;
    ...
    instructions.innerHTML = `
            <li>🔄 <strong>Rotate to landscape mode</strong> for better pattern visualization</li>
            <li>📱 <strong>Use tablet/desktop</strong> for optimal text chunking analysis</li>
            <li>🎯 <strong>Chunk patterns</strong> display better with wider screens</li>
            <li>📊 <strong>Regular expressions</strong> and results need horizontal space</li>
        `;
    ...
    features.innerHTML = `
            <div style="font-size: 0.85rem; color: #B0B0B0; margin-bottom: 1.5rem; font-style: italic;">
                ✨ Pattern recognition • Chunk extraction • IOB tagging • Phrase analysis
            </div>
        `;
    ...
    performanceTip.innerHTML = `
            <div style="font-size: 0.75rem; color: #666; margin-top: 0.8rem; padding: 0.8rem; background: rgba(255,255,255,0.05); border-radius: 8px; border-left: 3px solid #4FC3F7;">
                <strong>💡 Pro Tip:</strong> Regex patterns and chunk visualization work best on screens ≥1024px wide
            </div>
        `;
    ...
    debugLog("Chunking Alert element created");
    return overlay;
  }
  ...
  function showAlert() {
    ...
    debugLog("Showing Chunking orientation alert");
    ...
    if (typeof gtag === "function") {
      gtag("event", "chunking_mobile_orientation_alert_shown", {
        device_width: window.innerWidth,
        device_height: window.innerHeight,
        orientation: getOrientation(),
        experiment: "text_chunking",
      });
    }
  ...
  function hideAlert() {
    ...
    debugLog("Hiding Chunking orientation alert");
    ...
    // Track analytics if available
    if (typeof gtag === "function") {
      gtag("event", "chunking_mobile_orientation_alert_dismissed", {
        orientation: getOrientation(),
        experiment: "text_chunking",
  ...
  function checkOrientationAndShowAlert() {
    ...
    debugLog("Chunking Orientation check", {
  ...
  function handleOrientationChange() {
    debugLog("Chunking Orientation change detected");
  ...
  function handleResize() {
    debugLog("Chunking Window resize detected");
    checkOrientationAndShowAlert();
  }
  ...
  function startOrientationChecking() {
    ...
    debugLog("Started Chunking orientation checking interval");
  }
  ...
  function stopOrientationChecking() {
    if (orientationCheckInterval) {
      ...
      debugLog("Stopped Chunking orientation checking interval");
    }
  }
  ...
  function initialize() {
    ...
    debugLog("Initializing Chunking mobile detection system");
    ...
    isInitialized = true;
    debugLog("Chunking mobile detection system initialized");
  }

  /**
   * Public API for Chunking experiment
   */
  window.ChunkingMobileDetection = {
    init: initialize,
  ...
  debugLog("Chunking Mobile detection script loaded");
})();
