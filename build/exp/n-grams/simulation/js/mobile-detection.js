/**
 * Mobile Detection and Orientation Alert for N-grams Experiment
 * Alerts users on small screens to use landscape mode for better bigram visualization experience
 */
...
(function () {
  ...
  function debugLog(message, data = null) {
    if (CONFIG.enableDebugMode) {
      console.log(`[NGrams-MobileDetection] ${message}`, data || "");
    }
  }
  ...
  function hasUserDismissedAlert() {
    ...
    if (sessionStorage) {
      return sessionStorage.getItem("ngrams-mobile-alert-dismissed") === "true";
    }
  ...
  function markAlertAsDismissed() {
    ...
    if (sessionStorage) {
      sessionStorage.setItem("ngrams-mobile-alert-dismissed", "true");
    }
  ...
  function resetDismissalState() {
    ...
    if (sessionStorage) {
      sessionStorage.removeItem("ngrams-mobile-alert-dismissed");
    }
  ...
  function createAlertElement() {
    ...
    overlay.id = "ngrams-mobile-orientation-alert";
    ...
    // Add CSS animations
    if (!document.getElementById("ngrams-mobile-alert-styles")) {
      const style = document.createElement("style");
      style.id = "ngrams-mobile-alert-styles";
      style.textContent = `
                ...
                .ngram-animation {
                    animation: ngramFlow 3s ease-in-out infinite;
                }
    ...
    // N-gram icon representation
    const ngramIcon = document.createElement("div");
    ngramIcon.innerHTML = "📊";
    ngramIcon.style.cssText = `
            font-size: 4.5rem;
            margin-bottom: 1.2rem;
            display: block;
        `;
    ngramIcon.className = "ngram-animation";

    // Main heading
    const heading = document.createElement("h2");
    heading.textContent = "Bigram Tables Work Best in Landscape";
    ...
    expTitle.textContent = "N-grams Language Model";
    ...
    description.innerHTML = `
            This experiment involves <strong>bigram probability matrices</strong> and n-gram frequency calculations. For optimal experience with statistical tables and corpus analysis:
        `;
    ...
    instructions.innerHTML = `
            <li>🔄 <strong>Rotate to landscape mode</strong> for better table visualization</li>
            <li>📱 <strong>Use tablet/desktop</strong> for optimal bigram matrix viewing</li>
            <li>🎯 <strong>Probability tables</strong> require wider screen space</li>
            <li>📊 <strong>Corpus data</strong> and results display better horizontally</li>
        `;
    ...
    features.innerHTML = `
            <div style="font-size: 0.85rem; color: #B0B0B0; margin-bottom: 1.5rem; font-style: italic;">
                ✨ Bigram analysis • Frequency matrices • Corpus selection • Probability calculation
            </div>
        `;
    ...
    performanceTip.innerHTML = `
            <div style="font-size: 0.75rem; color: #666; margin-top: 0.8rem; padding: 0.8rem; background: rgba(255,255,255,0.05); border-radius: 8px; border-left: 3px solid #4FC3F7;">
                <strong>💡 Pro Tip:</strong> Bigram matrices and probability calculations work best on screens ≥1024px wide
            </div>
        `;

    // Assemble the content
    content.appendChild(ngramIcon);
    ...
    debugLog("N-grams Alert element created");
    return overlay;
  }
  ...
  function showAlert() {
    ...
    debugLog("Showing N-grams orientation alert");
    ...
    if (typeof gtag === "function") {
      gtag("event", "ngrams_mobile_orientation_alert_shown", {
        device_width: window.innerWidth,
        device_height: window.innerHeight,
        orientation: getOrientation(),
        experiment: "n_grams_language_model",
      });
    }
  ...
  function hideAlert() {
    ...
    debugLog("Hiding N-grams orientation alert");
    ...
    // Track analytics if available
    if (typeof gtag === "function") {
      gtag("event", "ngrams_mobile_orientation_alert_dismissed", {
        orientation: getOrientation(),
        experiment: "n_grams_language_model",
  ...
  function checkOrientationAndShowAlert() {
    ...
    debugLog("N-grams Orientation check", {
  ...
  function handleOrientationChange() {
    debugLog("N-grams Orientation change detected");
  ...
  function handleResize() {
    debugLog("N-grams Window resize detected");
    checkOrientationAndShowAlert();
  }
  ...
  function startOrientationChecking() {
    ...
    debugLog("Started N-grams orientation checking interval");
  }
  ...
  function stopOrientationChecking() {
    if (orientationCheckInterval) {
      ...
      debugLog("Stopped N-grams orientation checking interval");
    }
  }
  ...
  function initialize() {
    ...
    debugLog("Initializing N-grams mobile detection system");
    ...
    isInitialized = true;
    debugLog("N-grams mobile detection system initialized");
  }

  /**
   * Public API for N-grams experiment
   */
  window.NGramsMobileDetection = {
    init: initialize,
  ...
  debugLog("N-grams Mobile detection script loaded");
})();
